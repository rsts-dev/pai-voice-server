...
if curl -s -f http://localhost:8888/health > /dev/null 2>&1; then
    ...
    echo "Port: 8888"
    echo "---"
    echo "Stop Server | bash='pai-voice-server stop' terminal=false refresh=true"
    echo "Restart Server | bash='pai-voice-server restart' terminal=false refresh=true"
else
    ...
    echo "---"
    echo "Start Server | bash='pai-voice-server start' terminal=false refresh=true"
fi

echo "---"
echo "Check Status | bash='pai-voice-server status' terminal=true"
...
echo "---"
echo "Open Voice Server Folder | bash='open ~/.claude/pai-voice-server'"
echo "Uninstall | bash='pai-voice-server uninstall' terminal=true"